...
- “加载微信历史消息”：默认关闭；开启后可设置 `1–500` 条的加载上限，每页读取 20 条，只用于窗口展示，不进入 Agent 上下文。

聊天记录首次仅展示最新 20 条并自动置底；向上滚动到顶部（或点击顶部加载提示）后，每次加载更早的 20 条，保持当前阅读位置。Agent 数据库历史采用分页查询，不会一次性读取全部记录，且不受上述微信历史上限影响。阅读历史时，新消息只显示提示；滑到底部或点击新消息提示后再跟随最新消息。引用的原消息不在当前页时会按需加载定位；记录已清空则提示无法定位。翻页不会删除历史记录，也不会触发 Agent 或 Webhook。
